...
{
    public partial class RepSalesDeletedItems80mmForm : Form
    {
        public DateTime dateStart;
        public DateTime dateEnd;

        public RepSalesDeletedItems80mmForm(DateTime startDate, DateTime endDate)
        {
            InitializeComponent();
            dateStart = startDate;
            dateEnd = endDate;
            ...
            {
                printDocumentDeletedItem.DefaultPageSettings.PaperSize = new PaperSize("Official Receipt", 255, 38500);
            }
            else if (Modules.SysCurrentModule.GetCurrentSettings().PrinterType == "Thermal Printer")
            {
                printDocumentDeletedItem.DefaultPageSettings.PaperSize = new PaperSize("Official Receipt", 270, 38500);
            }
            else
            {
                printDocumentDeletedItem.DefaultPageSettings.PaperSize = new PaperSize("Official Receipt", 175, 38500);
            }
        }

        private void printDocumentDeletedItem_PrintPage(object sender, PrintPageEventArgs e)
        {
            // ============
            // Data Context
            // ============
            ...
            else
            {
                ...
                // 80mm Report Title
                // =================
                String title = "Deleted Item Report";
                ...
                String item = "ITEM";
                graphics.DrawString(item, fontArial8Bold, drawBrush, new RectangleF(x, y + 5, width, height), drawFormatLeft);
                //y += graphics.MeasureString(item, fontArial8Regular).Height;

                var deletedItems = from s in db.TrnSalesLineDeleteds
                                   where s.SalesDate >= dateStart
                                    && s.SalesDate <= dateEnd
                                    && s.IsPrinted == true
                                   select s;
                ...
                if (deletedItems.Any())
                {
                    ...
                    {
                        String itemData = deletedItem.ItemDescription + "\n" + deletedItem.Quantity.ToString("#,##0.00") + " " + deletedItem.Unit + " " + deletedItem.Preparation + " " + deletedItem.SalesNumber;
        ...
        {
            printDocumentDeletedItem.Print();
        }
